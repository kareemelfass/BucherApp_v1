...
ui <- navbarPage(
  ...
  tabPanel(
    ...
    fluidRow(
      ...
      # --------- UPDATED NOTES COLUMN (includes RD block) ---------
      column(
        7,
        h3("Notes"),
        tags$ul(
...
server <- function(input, output, session) {
  ...
  parsed_inputs <- reactive({
    ...
    # Fix direction to (A vs C) and (B vs C)
    if (input$dir_AC == "C_A") {
  ...
  results <- eventReactive(input$btn_run, {
    ...
    # Warn (but do not block) if assumptions are violated
    failed <- names(st$checks)[!st$checks]
    if (length(failed) > 0) {
      shinyalert(
    ...
    # Additional informational popup for RD
    if (pin$measure == "RD") {
      shinyalert(
        title = "RD caution",
  ...
  output$plot_network <- renderPlot({
    ...
    legend("bottom",
           legend = c("Direct evidence", "Indirect comparison path"),
